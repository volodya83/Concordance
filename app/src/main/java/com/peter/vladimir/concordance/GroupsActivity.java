...
public class GroupsActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private ImageButton ibtn_group_index;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ibtn_group_index = (ImageButton) findViewById(R.id.ibtn_group_index);
        ibtn_group_index.setOnClickListener(this);
        ...
        refreshFirstList();
        tv_grp_name_title.setText("Group name");
    ...
    public void onClick(View v) {
        ...
        if (v != ibtn_grp_plus && v != ibtn_group_index) {
            selectedRbtn = v;
        }
        if (v == rbtn_grp_phrase) {
            ...
            if (selectedRbtn == rbtn_grp_group) {
                if (grpStr.length() > 0 && grpName.length() > 0) {
                    ...
                    Toast.makeText(this, "Group or word is not correct", Toast.LENGTH_SHORT).show();
                ...
                } else Toast.makeText(this, "Pair is not correct", Toast.LENGTH_SHORT).show();
            } else if (selectedRbtn == rbtn_grp_phrase) {
                if (grpStr.length() > 0) {
                    SQLfunctions.insertPhrase(grpStr);
                    refreshSecondList(0, "", 0);
                } else Toast.makeText(this, "Phrase is not correct", Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    private void changeLayout(int status) {
        if (status == GROUP || status == RELATION) {
            ...
            if (status == GROUP) {
                tv_grp_name_title.setText("Group name");
                et_grp_str.setHint("Enter word to group");
                params = ibtn_group_index.getLayoutParams();
                params.height = ViewGroup.LayoutParams.WRAP_CONTENT;
                ibtn_group_index.setLayoutParams(params);
            } else {
                tv_grp_name_title.setText("Relation name");
                et_grp_str.setHint("Enter word1-word2 to relation");
                params = ibtn_group_index.getLayoutParams();
                params.height = 0;
                ibtn_group_index.setLayoutParams(params);
            }
            ...
            lv_grp_2lst.setLayoutParams(params);
            et_grp_str.setHint("Enter phrase");
            ibtn_group_index.setEnabled(false);
        }
    }
        ...
        public void onClick(View v) {
            if (selectedRbtn == rbtn_grp_group) {
                if (v.getId() == R.id.tv_grp_item_name) {
                    ...
                    refreshSecondList(R.id.tv_grp_item_name, _group_name, 1);
